...
BEFORE_OFFSET=$(docker compose exec -T kafka kafka-run-class.sh kafka.tools.GetOffsetShell --broker-list localhost:9092 --topic alerts --time -1 2>/dev/null | awk -F: '{sum+=$3} END{print sum+0}')
...
AFTER_OFFSET=$(docker compose exec -T kafka kafka-run-class.sh kafka.tools.GetOffsetShell --broker-list localhost:9092 --topic alerts --time -1 2>/dev/null | awk -F: '{sum+=$3} END{print sum+0}')
