...
ALERT_BODY=$(jq -nc --argjson sid "$SERVICE_ID" '{serviceId:$sid,level:"ERROR",threshold:1,windowMinutes:10}')
ALERT_RESP=""
ALERT_ID=""
for i in {1..12}; do
...
for i in 1 2; do
  ...
  [[ "$CODE" = "202" || "$CODE" = "200" ]] || { echo "INGEST_FAIL_$i code=$CODE body=$(cat /tmp/ingest_resp.json)"; exit 1; }
  sleep 1
done
